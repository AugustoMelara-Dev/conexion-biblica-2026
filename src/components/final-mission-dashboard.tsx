import { Clock3, Flag, ShieldCheck, Target } from "lucide-react"

import { buildFinalMissionPlan, getNextMission, type FinalMission } from "@/domain/final-mission-plan"
...
export function FinalMissionDashboard({
  ...
  onContinue: (mission: FinalMission) => void
  onManual?: () => void
}) {
  ...
  const remainingHours = Math.max(0, Math.ceil((competition - now.getTime()) / 3_600_000))
  const progress = plan.length ? Math.round((plan.filter((mission) => completed.has(mission.id)).length / plan.length) * 100) : 0

  if (!next) return null
  return (
    <section className="overflow-hidden rounded-3xl border border-primary/20 bg-[linear-gradient(135deg,hsl(var(--primary)/0.12),hsl(var(--card))_58%)] shadow-sm" aria-labelledby="final-mission-title">
      <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[minmax(0,1.25fr)_minmax(17rem,0.75fr)] lg:p-10">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <Badge className="gap-1"><Flag className="size-3.5" aria-hidden="true" /> V5 — Consolidación Final</Badge>
          ...
          <p className="mt-3 max-w-2xl text-base leading-7 text-muted-foreground">Una misión a la vez. La prioridad es recordar hechos GOLD después de un intervalo, no recorrer variantes.</p>

          <div className="mt-7 rounded-2xl border border-border/70 bg-background/80 p-5 sm:p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">Próxima misión</p>
            <h2 className="mt-2 text-xl font-semibold">{next.label}</h2>
            <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">{next.description}</p>
            <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm">
              <span className="inline-flex items-center gap-2"><Target className="size-4 text-primary" aria-hidden="true" />{next.count} preguntas</span>
              <span className="inline-flex items-center gap-2"><Clock3 className="size-4 text-primary" aria-hidden="true" />~{next.durationMinutes} min</span>
              <span className="inline-flex items-center gap-2"><ShieldCheck className="size-4 text-primary" aria-hidden="true" />{next.blindPool ? `Ciega ${next.blindPool}` : "Solo GOLD"}</span>
            </div>
          </div>
        ...
        <aside className="rounded-2xl bg-secondary/55 p-5 sm:p-6" aria-label="Progreso de hoy">
          ...
          <p className="mt-7 text-xs leading-5 text-muted-foreground">Las repeticiones inmediatas reparan; solo las recuperaciones separadas aumentan dominio.</p>
        </aside>
      </div>
    </section>
  )
