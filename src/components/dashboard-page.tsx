...
export function DashboardPage({
  ...
  const currentStreak = progress.size
    ? Math.max(
        ...[...progress.values()].map((item) => item.currentCorrectStreak),
        0
      )
    : 0
  ...
  const masteredFacts = factMastery.filter(
    (fact) => fact.state === "mastered"
  ).length
  ...
  return (
    <div className="flex min-w-0 flex-col gap-10">
      ...
      <MetricStrip
        items={[
          {
            label: "Precisión fría",
            value: `${evidenceAccuracy("cold")}%`,
            detail: "Primer intento sin feedback",
            icon: Target,
          },
          {
            label: "Precisión diferida",
          ...
          },
          {
            ...
            value: masteredFacts,
            detail: `${factMastery.length} hechos con evidencia`,
            icon: Check,
          },
        ]}
      />
      ...
      <section
        className="rounded-2xl bg-secondary/45 px-5 py-6 sm:px-7"
        aria-labelledby="canonical-bank-title"
      >
        <p className="text-sm font-medium text-primary">Fuente activa</p>
        <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2
              id="canonical-bank-title"
              className="text-2xl font-semibold tracking-tight"
            >
              Banco Maestro Único — Final 2026
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              Un solo historial, cobertura por hecho y tres tipos visibles:
              completar, verdadero/falso y selección única.
            </p>
          </div>
        ...
        items={[
          {
            ...
            detail: `${general.correct} correctas de ${general.total}`,
            icon: Target,
          },
          {
            label: "Preguntas respondidas",
            value: general.total,
            detail: `${general.seen} apariciones registradas`,
          },
          {
            label: "Tiempo promedio",
            value: formatElapsedMs(general.averageResponseTimeMs),
            detail: `Mejor ${formatElapsedMs(general.bestResponseTimeMs)}`,
            icon: Clock3,
          },
          {
            label: "Racha actual",
            value: currentStreak,
            detail: `${sessions.length} sesiones realizadas`,
            icon: Flame,
          },
        ]}
      />
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.85fr)]">
        <section
          aria-label="Rendimiento por fuente"
          className="min-w-0 rounded-2xl bg-secondary/45 p-6 sm:p-8"
        >
          <SectionHeader
        ...
        <section
          ...
          className="rounded-2xl border border-border/70 p-6 sm:p-8"
        >
          ...
          <div className="mt-8 flex items-end justify-between gap-4">
            <div>
              <p className="text-3xl font-semibold tabular-nums">
                {questions.length
                  ? Math.round((general.seen / questions.length) * 100)
                  : 0}
                %
              </p>
      ...
      <section aria-label="Rendimiento por capítulo" className="min-w-0">
        <SectionHeader
          ...
          description="Ordenado de peor a mejor precisión."
        />
            ...
            <TableBody>
                ...
                <TableRow key={row.key}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <span className="tabular-nums">{row.accuracy}%</span>
                      <Progress
                        aria-label={`Precisión de ${row.label}`}
                        className="w-20"
                        value={row.accuracy}
                      />
                    </div>
                  ...
                  <TableCell className="tabular-nums">
                    {formatElapsedMs(row.averageResponseTimeMs)}
                  </TableCell>
                  <TableCell>
              ...
              {statistics.chapters.length === 0 ? (
                <TableRow>
                  ...
                  >
                    Aún no hay capítulos con preguntas cargadas.
                  </TableCell>
                </TableRow>
              ) : null}
            </TableBody>
      ...
      >
        ...
        Guardado local: banco maestro único · {questions.length} preguntas
        cargadas · IndexedDB activo.
      </p>
    </div>
  )
}
