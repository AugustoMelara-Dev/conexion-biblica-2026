...
function statusMatches(question: Question, progress: QuestionProgress | undefined, statuses: SessionConfig["statuses"]) {
  ...
  return statuses.some((status) => {
    ...
    if (status === "failed") return Boolean(progress && progress.timesIncorrect > 0)
    if (status === "difficult") return question.difficulty >= 4 || Boolean(progress?.markedDifficult)
...
function baseScore(question: Question, progress: QuestionProgress | undefined, mode: SessionConfig["mode"], recentBonus = 0) {
  ...
  if (mode === "errors") return failed * 100 + (progress?.markedDifficult ? 60 : 0) + slow + question.difficulty * 4 + recentBonus * 2
...
export function selectSessionQuestions(
  ...
  const championshipQuotas = config.mode === "championship" || config.mode === "simulation" ? [
    ...
    { matches: (question: Question) => question.difficulty <= 2, remaining: Math.max(0, target - Math.round(target * 0.4) - Math.round(target * 0.35) - Math.round(target * 0.2)) },
  ] : null
  while (result.length < target && pool.length > 0) {
    let eligiblePool = pool
    const preferredQuota = championshipQuotas?.find((quota) => quota.remaining > 0 && pool.some((candidate) => quota.matches(candidate.question)))
    const redistributedQuota = preferredQuota ?? championshipQuotas?.find((quota) => pool.some((candidate) => quota.matches(candidate.question)))
    if (redistributedQuota) {
      eligiblePool = pool.filter((candidate) => redistributedQuota.matches(candidate.question))
      if (preferredQuota) preferredQuota.remaining -= 1
    }
    eligiblePool.sort((left, right) => right.score - left.score)
    const preferred = eligiblePool.find((candidate) => !questionsShareFacts(candidate.question, lastQuestion) && candidate.question.source.chapter !== lastChapter)
    const fallback = eligiblePool.find((candidate) => !questionsShareFacts(candidate.question, lastQuestion))
...
export function filterEligibleQuestions(
  ...
  return questions.filter((question) => {
    const itemProgress = progress.get(`${question.bankId ?? "local"}:${question.id}`) ?? progress.get(question.id)
    if (config.sourceWorks.length && !config.sourceWorks.includes(question.source.work)) return false
    if (config.chapters.length && !config.chapters.includes(question.source.chapter)) return false
 ...
 */
export function scheduleTrainingRetry(queue: Question[], question: Question, currentIndex: number, waitFor: number): Question[] {
  const key = `${question.bankId ?? "local"}:${question.id}`
  if (queue.some((item, index) => index > currentIndex && `${item.bankId ?? "local"}:${item.id}` === key)) return queue
