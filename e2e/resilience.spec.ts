...
test("el respaldo completo se descarga como JSON versionado", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "Una descarga integral basta para validar el contrato"
  )
...
test("restaura de verdad un respaldo con mil sesiones y lo conserva al recargar", async ({
  ...
  await expect(
    page.getByText("Respaldo restaurado.", { exact: false })
  ).toBeVisible({
    timeout: 240_000,
  })
  ...
  await expect(
    page.getByRole("list", { name: "Sesiones guardadas" })
  ).toBeVisible()
...
test("el shell vuelve a abrir sin red después de quedar bajo control del service worker", async ({
  ...
  test.skip(
    ...
    "Una validación por motor es suficiente"
  )
  await waitForHome(page)
...
test("una red lenta al cargar el banco no rompe la pantalla inicial", async ({
  page,
}, testInfo) => {
  test.skip(
    !testInfo.project.name.startsWith("desktop-"),
...
test("una cuota agotada al guardar una preferencia no derriba la navegación", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== "desktop-chromium",
    "La ruta de almacenamiento es común a los motores"
  )
