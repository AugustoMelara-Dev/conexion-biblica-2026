...
def main() -> None:
    ...
    lines = [
        ...
        f"- Reemplazos editoriales nuevos desde el PDF: {report['generated_editorial_replacements']}",
        ...
        *[f"- {chapter}: {count}" for chapter, count in chapters.items()],
        ...
        "## GOLD por tipo",
        "",
        *[f"- {question_type}: {count}" for question_type, count in report['gold_by_type'].items()],
        "",
        "## Contrato obligatorio",
        "",
        f"- Errores de mezcla o cuota: {len(report['mandatory_mix_errors'])}",
        f"- Duplicados semánticos eliminados: {report['duplicate_variants_removed']}",
        f"- Reserva ciega A/B/emergencia: {report['blind_pools']}",
        "",
