#!/usr/bin/env python3
"""
Updates progress.json and progress.md after Wave 4 promotion and Pilot R3 V2 evaluation.
"""
import json
import os
import pathlib
...
def update_progress():
    ...
    progress_data = {
        ...
        "public_questions": manifest["gold_questions"], # 3,873
        "total_canonical_facts": manifest["unique_facts"], # 2,217
        "shards_count": len(manifest["shards"]),
        "r2_unique_facts_covered": 1391, # 1,210 + 181
        "r2_unique_facts_remaining": 826, # 1,007 - 181
        "r3_candidates_generated": 120, # 60 Pilot 1 + 60 Pilot 2
        "r3_competitive_accept": 53, # 53 from Pilot 2
        "r3_downgraded_to_coverage": 146, # 80 Wave 2 + 59 Pilot 1 + 7 Pilot 2
        "r3_rewrite": 0,
        "r3_reject": 1,
        "translation_noise_competitive_accept": 9,
        "deployments": [
            {
                "deployment_id": "dpl_3zoWpK8yoj6x2Gh36WPcd2i4UoNb",
                "public_questions": manifest["gold_questions"],
                "shards": 18,
                "failures": 0,
        ...
        "current_status": "WAVE_4_R2_PROMOTED_PILOT_R3_V2_AUDITED"
    }
